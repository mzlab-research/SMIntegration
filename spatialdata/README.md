# Processed Data and Figure Mapping

This directory provides a mapping between the processed RDS data files used for benchmarking and the figures presented in the SMIntegration manuscript.

## Data Directory
All processed and renamed data files for easy identification are located in the `spatialdata/data/` directory.

## Figure to Data Mapping

### Main Figures

| Figure | Description | Renamed Data File (in `spatialdata/data/`) |
| :--- | :--- | :--- |
| **Figure 3a** | Spatial Group Assignment (NA vs MO) | `fig3a_group_assignment.rds` |
| **Figure 3b-c** | UMAP Visualization (Metab & Trans) | `fig3bc_umap_data.rds` |
| **Figure 3d** | Differential Feature Counts | `fig3d_feature_counts.rds` |
| **Figure 3e-f** | Differential Correlation Networks | `fig3ef_correlation_networks.rds` |
| **Figure 4a** | Spatial Group Assignment (CA vs PAG) | `fig4a_group_assignment.rds` |
| **Figure 4b** | Differential Feature Counts | `fig4b_feature_counts.rds` |
| **Figure 4c** | Pathway Overlap Venn Diagram | `fig4c_pathway_venn.rds` |
| **Figure 4d** | Pathway Annotation Dot Plot | `fig4d_pathway_dotplot.rds` |
| **Figure 4e-f** | Pathway Map & Feature Distribution | `fig4ef_kegg_map_data.rds`,`fig4f_data_rds.rds` |
| **Figure 4g** | Multi-Modal Co-Visualization | `fig4g_co_visualization.rds` |

### Supplementary Figures

| Figure | Description | Generation Method |
| :--- | :--- | :--- |
| **Supp Fig 1-10** | SMIntegration Platform Screenshots | Generated by loading the example mouse brain dataset (`figS1-10_SM.rds`,figS1-10_ST.rds) into the [SMIntegration Web Server](https://metax.genomics.cn/app/smintegration). |

## How to Reproduce
The scripts to reproduce the figures (3-4) are located in "spatialdata/script/". You can run "figure3.R" and "figure4.R" locally. Note that original scripts use the relative path "../data/".

## License
The processed data in this repository is provided under the **CC0 license**.
